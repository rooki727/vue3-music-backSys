...
import { getPlaylistListAPI, deleteBatchPlaylistAPI } from '@/apis/playlist'
// 搜索功能变量
...
const getTableForm = async () => {
  // 请求接口
  const res = await getPlaylistListAPI()
  if (res.code === 200) {
...
const resetSearch = () => {
  searchInput.value = ''
  playList.value = [...originalData.value]
}
...
const handleSearch = (inputvalue) => {
  ...
  debounceTimer = setTimeout(() => {
    ...
    if (inputvalue === '') {
      ...
      // 根据输入值过滤数据
      const filteredData = originalData.value.filter((item) => item?.title?.includes(inputvalue))
      // 更新表格数据
...
const blukDel = async () => {
  if (delTableId.value.length > 0) {
    // 执行请求操作
    await deleteBatchPlaylistAPI(delTableId.value)
